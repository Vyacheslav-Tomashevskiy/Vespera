...
export default function FilesPage() {
  ...
  const filteredFiles = useMemo(() => {
    ...
    if (filterType !== 'all') {
      result = result.filter(f => {
        ...
        if (filterType === 'archive') return f.fileType.includes('zip') || f.fileType.includes('rar') || f.fileType.includes('tar');
        if (filterType === 'document') return f.fileType.includes('word') || f.fileType.includes('sheet') || f.fileType.includes('excel') || f.fileType.includes('pdf');
        return true;
      });
    }
  ...
  const submitRename = async () => {
    ...
    try {
      const updated = await storageApi.updateMetadata(selectedFile.s3Key, renameValue);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            {[
                ...
                { label: 'Total Storage', value: `${(files.reduce((a, b) => a + b.fileSize, 0) / (1024 * 1024)).toFixed(1)} MB`, color: 'purple' },
                { label: 'Images', value: files.filter(f => f.fileType.startsWith('image/')).length, color: 'green' },
                { label: 'Documents', value: files.filter(f => f.fileType === 'application/pdf').length, color: 'orange' },
            ].map((stat, i) => (
                <div key={i} className="bg-white dark:bg-neutral-800 rounded-3xl p-6 border border-neutral-100 dark:border-neutral-700 shadow-sm">
                    <p className="text-sm font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-1">{stat.label}</p>
        ...
        {loading && files.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 bg-white dark:bg-neutral-800 rounded-3xl border border-neutral-100 dark:border-neutral-700">
            ...
            <p className="text-neutral-600 dark:text-neutral-400 font-bold">Loading your files...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-white dark:bg-neutral-800 rounded-3xl border border-dotted border-neutral-300 dark:border-neutral-700">
            ...
            <h3 className="text-xl font-black text-neutral-900 dark:text-white mb-2">No files found</h3>
      ...
      {isRenameOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-neutral-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-neutral-800 rounded-3xl w-full max-w-md shadow-2xl border border-neutral-200 dark:border-neutral-700 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8">
              <h2 className="text-2xl font-black text-neutral-900 dark:text-white mb-6">Rename File</h2>
              <div className="mb-8">
                <label className="block text-sm font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest mb-3">New Name</label>
                <input
                  type="text"
                  value={renameValue}
